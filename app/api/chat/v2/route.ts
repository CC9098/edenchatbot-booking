...
const FALLBACK_CONSTITUTION_CONTEXT: Record<ConstitutionType, string> = {
  depleting:
    '用戶屬於「虛耗型」體質。重點關注氣血不足、津液虧虛的問題。建議方向包括補氣養血、滋陰潤燥、健脾益腎。避免過度勞累和寒涼飲食。',
  crossing:
    '用戶屬於「交錯型」體質。重點關注寒熱錯雜、虛實夾雜的複合狀態。建議方向包括平衡寒熱、扶正祛邪、調和陰陽。需要根據具體症狀靈活調整。',
  hoarding:
    '用戶屬於「屯積型」體質。重點關注痰濕、瘀血、食積等停滯問題。建議方向包括化痰祛濕、活血化瘀、消食導滯。鼓勵適量運動和清淡飲食。',
  mixed:
...
- 涉及診所電話時，如該診所有 WhatsApp，必須同時提供可點擊 WhatsApp 連結。
- 回覆診所聯絡資料時，只可使用「診所資訊」或「WhatsApp 聯絡」段落內的最新號碼與連結；荃灣診所電話必須包含 2698 5422，如需提供 WhatsApp，使用 5189 9065 / https://wa.me/85251899065。
- 涉及診所 WhatsApp 或地址/地圖連結時，優先用 Markdown 超連結格式，不要直接顯示 raw URL；只有在明顯不支援超連結時，先輸出完整 https://wa.me/... 或 Google Maps URL。
...
- 必須提供可點擊 Google 地圖連結；如連結太長，優先用 Markdown 超連結格式，例如 [中環診所 Google 地圖](https://...)。
- 如同時提供 WhatsApp，亦優先用 Markdown 超連結格式，例如 WhatsApp：[按此聯絡姑娘](https://wa.me/...)。
