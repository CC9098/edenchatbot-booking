...
export async function PATCH(
  ...
  try {
    ...
    const holidayId = params.holidayId?.trim();
    if (!holidayId) {
    ...
    if (error || !data) {
      console.error('[PATCH /api/doctor/timetable/holidays/[holidayId]] update error:', error?.message);
    ...
    console.error('[PATCH /api/doctor/timetable/holidays/[holidayId]] unexpected error:', error);
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error('[DELETE /api/doctor/timetable/holidays/[holidayId]] delete error:', error.message);
    ...
    console.error('[DELETE /api/doctor/timetable/holidays/[holidayId]] unexpected error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
